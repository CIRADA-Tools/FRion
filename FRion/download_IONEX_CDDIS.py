...
def get_CDDIS_IONEXfile(
    time="2023/03/23/02:20:10.01", prefix="jplg", outpath="./", overwrite=False
):
    ...
    if not os.path.isdir(outpath):
        try:
            ...
            print("cannot create output dir for IONEXdata: %s", outpath)
    ...
    if not overwrite and os.path.isfile(
        "%s%s%03d0.%02dI" % (outpath, prefix.upper(), dayofyear, yy)
    ):
    ...
    if prefix == "jplg" and mydate > datetime.date(2023, 8, 7):
        filename = (
            f"{prefix[0:3].upper()}0OPSFIN_{year}{dayofyear}0000_01D_02H_GIM.INX.gz"
        )
    elif prefix == "codg" and mydate > datetime.date(2022, 11, 26):
        filename = (
            f"{prefix[0:3].upper()}0OPSFIN_{year}{dayofyear}0000_01D_01H_GIM.INX.gz"
        )
        ...
        filename = (
            f"{prefix[0:3].upper()}0OPSFIN_{year}{dayofyear}0000_01D_30M_GIM.INX.gz"
        )
    elif prefix == "esag":
        if mydate <= datetime.date(2023, 2, 4):
            raise Exception(
                "ESA did not publish GIM.INX.gz files prior to 2023 Feb 04."
            )
        else:
            filename = (
                f"{prefix[0:3].upper()}0OPSFIN_{year}{dayofyear}0000_01D_02H_GIM.INX.gz"
    ...
    url = "https://cddis.nasa.gov/archive/gnss/products/ionex/%4d/%03d/%s" % (
        year,
        dayofyear,
        filename,
    )
