...
def apply_correction_to_files(
    ...
    # Save results
    write_corrected_cubes(
        Qoutfile, Uoutfile, Qcorr, Ucorr, Qheader, Uheader, overwrite=overwrite
    )
...
def write_corrected_cubes(
    Qoutputname, Uoutputname, Qcorr, Ucorr, Qheader, Uheader, overwrite=False
):
    ...
    Qoutput_header.add_history(
        "Corrected for ionospheric Faraday rotation using FRion."
    )
    Uoutput_header = Uheader.copy()
    Uoutput_header.add_history(
        "Corrected for ionospheric Faraday rotation using FRion."
    )
    ...
    pf.writeto(
        Qoutputname, Qcorr.astype("float32"), Qoutput_header, overwrite=overwrite
    )
    pf.writeto(
        Uoutputname, Ucorr.astype("float32"), Uoutput_header, overwrite=overwrite
    )
...
def apply_correction_large_cube(
    ...
    shape = tuple(
        Qoutput_header["NAXIS{0}".format(ii)]
        for ii in range(1, Qoutput_header["NAXIS"] + 1)
    )

    Qoutput_header.tofile(Qoutfile)
    with open(Qoutfile, "rb+") as fobj:
        fobj.seek(
            len(Qoutput_header.tostring())
            + (np.product(shape) * np.abs(Qoutput_header["BITPIX"] // 8))
            - 1
        )
        fobj.write(b"\0")
    ...
    with open(Uoutfile, "rb+") as fobj:
        fobj.seek(
            len(Uoutput_header.tostring())
            + (np.product(shape) * np.abs(Uoutput_header["BITPIX"] // 8))
            - 1
        )
        fobj.write(b"\0")
    ...
    for i in range(theta.size):  # Iterate over channels.
        if (N_dim == 4) & (freq_axis == 3):
            Pdata = Qdata[:, i] + 1.0j * Udata[:, i]  # Input complex polarization
        else:
            Pdata = Qdata[i] + 1.0j * Udata[i]  # Input complex polarization
